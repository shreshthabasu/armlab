    ...
    def run(self):
        ...
        while True:
            ...
            if self.rexarm.initialized:
                ...
                self.updateEndEffectorReadout.emit(
...
class Gui(QMainWindow):
    ...
    def __init__(self, parent=None):
        ...
        def nxt_if_arm_init(next_state): return self.sm.set_next_state(
        ...
        self.ui.btnUser2.clicked.connect(self.learn)
        self.ui.btnUser3.clicked.connect(self.remember)
        self.ui.btnUser4.clicked.connect(self.find_blocks)
        self.ui.btnUser5.clicked.connect(self.get_color)
        self.ui.btnUser12.clicked.connect(self.write)
        self.ui.btn_task2.clicked.connect(self.execute_tp)
    ...
    def learn(self):
        self.sm.set_next_state("learn")

    def remember(self):
        self.sm.set_next_state("remember")

    def repeat(self):
        self.sm.set_next_state("repeat")
    
    def get_color(self):
        self.sm.set_next_state("get_color")

    def find_blocks(self):
        self.sm.set_next_state("find_blocks")

    def write(self):
        self.sm.set_next_state("write")
    ...
    def sliderChange(self):
        ...
        if self.rexarm.initialized:
            self.rexarm.set_torque_limits(
                [self.ui.sldrMaxTorque.value() / 100.0] * self.rexarm.num_joints)
            self.rexarm.set_speeds_normalized_all(
                self.ui.sldrSpeed.value() / 100.0)
    ...
    def trackMouse(self, mouse_event):
        ...
        """
        pt = mouse_event.pos()
        d = self.kinect.DepthFrameRaw[pt.y(), pt.x()]
        d = self.kinect.convertDepthToSI(d)
        mouse_text = "("+str(pt.x())+","+str(pt.y())+","+str(d)+")"
        world_text = "(-,-,-)"
        if self.kinect.kinectCalibrated:
            world = self.kinect.calculatePixelToWorld(pt.x(), pt.y(), d)
            world_text = "("+str(world[0, 0])+", " + \
                str(world[1, 0])+", "+str(self.kinect.max_depth - d)+")"
            # a,b,c =
            # world_text = "({:.3f},{:.3f},{:.3f})".format(tuple(self.kinect.calculatePixelToWorld(pt.x(), pt.y(), d)))
            # print(world_text)
        if self.kinect.DepthFrameRaw.any() != 0:
            self.ui.rdoutMousePixels.setText(mouse_text)
            self.ui.rdoutMouseWorld.setText(world_text)
